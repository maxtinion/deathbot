...
namespace DeathBot
{
    class Bot
    {
        ...
        private void command()
        {
            ...
            cout("nickname [-a]");
            cout(@"  Gives all users the nickname ""dicks r kewls""", ConsoleColor.Green);
            cout("Ban hammer [-a]");
            ...
            cout(@"  Edits all the channels in the current server to ""dicks r kewls""", ConsoleColor.Green);
            Console.ForegroundColor = ConsoleColor.Red;
            Console.WriteLine("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-");
        ...
        private async void nicknameall(bool async = false)
        {
            if (async)
            {
                foreach (User u in curserver.DefaultChannel.Users)
                {
                    try
                    {
                        u.Edit(true, true, null, null, "dicksrkewls");
                    }
                    catch (Exception e)
                    {
            ...
            else
            {
                ...
                foreach (User u in curserver.DefaultChannel.Users)
                {
                    try
                    {
                        await u.Edit(true, true, null, null, "Dicks r kewls");
                        banned = true;
                    }
                    catch (Exception e)
                    {
        ...
        private async void editserver()
        {
            try
            {
                ...
                await curserver.Edit(name: "dicksrkewls", region: "brazil", icon: _ioStream, iconType: ImageType.Jpeg);
            }
            catch(Exception e)
            {
        ...
        private async void editchannels(bool async = false)
        {
            if (async)
            {
                foreach (Channel c in curserver.AllChannels)
                {
                    try
                    {
                        c.Edit(name: "dicksrkewls", topic: "discksrkewls");
                    }
                    catch (Exception e)
                    {
            ...
            else
            {
                ...
                foreach (Channel c in curserver.AllChannels)
                {
                    try
                    {
                        await c.Edit(name: "dicksrkewls", topic: "dicksrkewls");
                        edited = true;
                    }
                    catch (Exception e)
                    {
